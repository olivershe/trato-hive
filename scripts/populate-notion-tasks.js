...
function parsePhase6Tasks() {
  ...
  while ((match = taskRegex.exec(phase6Content)) !== null) {
    ...
    if (name.includes('Migration') || name.includes('Seed') || name.includes('Prisma')) {
      ...
      ['TASK-008', 'TASK-009', 'TASK-010', 'TASK-011', 'TASK-012', 'TASK-013'].includes(taskId)
    ) {
      currentPackage = 'auth'
    }
...
async function main() {
  if (!process.env.NOTION_API_KEY || !process.env.NOTION_TASKS_DB_ID) {
    ...
    console.error(
      'Usage: NOTION_API_KEY=secret_XXX NOTION_TASKS_DB_ID=xxx node populate-notion-tasks.js'
    )
    process.exit(1)
  }
